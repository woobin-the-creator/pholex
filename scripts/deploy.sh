...
if [[ "$MODE" == "prod" ]]; then
  ...
  # frontend 빌드는 docker 멀티스테이지(Dockerfile prod-dist)에서 수행된다.
  # 호스트 npm 단계가 없으므로 git pull 후 deploy.sh --prod 한 번으로 끝난다.
  echo "[deploy] prod stack up — frontend dist 는 docker 빌드 안에서 생성 (project=$PROJECT, env-file=$ENV_FILE)"
