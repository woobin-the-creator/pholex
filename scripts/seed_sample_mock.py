...
if __name__ == "__main__":
    # 실행 가드: mock 전용 시드 스크립트가 사내/운영 DB에서 실수로 돌지 않도록 막는다.
    # 일회용 opt-in 환경변수를 명령 앞에 직접 붙여야만 실행된다 (.env 에 두지 말 것).
    #   PHOLEX_ALLOW_MOCK_SEED=1 python scripts/seed_sample_mock.py
    if os.getenv("PHOLEX_ALLOW_MOCK_SEED") != "1":
        print(
            "거부: 이건 mock 전용 시드 스크립트예요 (합성 20k행을 'sample' 테이블에 적재).\n"
            "실수로 실제 데이터 환경에서 도는 걸 막기 위해 기본 차단돼 있어요.\n"
            "정말 실행하려면 일회용 플래그를 붙이세요:\n"
            "    PHOLEX_ALLOW_MOCK_SEED=1 python scripts/seed_sample_mock.py",
            file=sys.stderr,
        )
        sys.exit(1)
