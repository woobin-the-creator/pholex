...
from app.ports.dto import LotChangeEventDTO, LotRowDTO, LotStatusLiteral
...
# 사내 raw enum(lot_status_seg) → domain canonical.
_STATUS_MAP: dict[str, LotStatusLiteral] = {
    "Active": "run",
    "Hold": "hold",
    "PreActive": "wait",
}
_HOLD_RAW = "Hold"  # fetch_my_holds가 거르는 raw 값


def _map_status_to_canonical(raw: str) -> LotStatusLiteral:
    canonical = _STATUS_MAP.get(raw)
    if canonical is None:
        # 미지의 값은 wait로 떨어뜨리되 로그를 남겨 새 enum 등장을 탐지한다.
...
class PgSampleLotSource:
    ...
    def _diff(
        ...
        for lot_id, row in current.items():
            ...
            if prev is None:
                events.append(self._event(lot_id, "hold", None, "hold", row.hold_comment, row))
            elif prev.hold_comment != row.hold_comment:
                events.append(self._event(lot_id, "comment", "hold", "hold", row.hold_comment, row))
        for lot_id, prev in previous.items():
            if lot_id not in current:
                events.append(self._event(lot_id, "removed", "hold", None, None, prev))
        return events
    ...
    def _event(
        ...
        lot_id: str,
        change_type: str,
        previous_status: LotStatusLiteral | None,
        new_status: LotStatusLiteral | None,
    ...
    def _to_dto(self, row, viewer_employee_number: str) -> LotRowDTO:
        ...
        return LotRowDTO(
            lot_id=row.lot_id,
            status=_map_status_to_canonical(row.lot_status_seg),
