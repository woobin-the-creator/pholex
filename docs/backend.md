...
> **가정 명시**: 위 폴링 구조는 MES/EAP 소스가 REST API 또는 DB 직접 접근 방식이고 이벤트 Push를 지원하지 않는다고 가정한다.
> MES/EAP DB 직접 접근이 확정되는 경우, Step 1~2를 **PostgreSQL `LISTEN/NOTIFY`** 또는 **CDC(Change Data Capture, e.g. Debezium)** 방식으로 전환하여 폴링 주기를 없애고 변경 즉시 감지로 개선할 수 있다. Adapter 패턴(`collectors/base.py`)이 이 전환을 흡수한다.
